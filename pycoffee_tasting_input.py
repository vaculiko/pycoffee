...
layoutBeans = [
    ...
    [sg.Frame('', layout=[
     ...
     [sg.Column(layout=[BeansOrigin(k, d) for k, d in [('Country', "Země - Kenya, Brazil..."), ('Name', "Jméno - Kiwami, Diamond..."), ('Roaster',
                "Pražírna - Motmot, Father's..."), ('Processing', "Zpracování - natural, washed..."), ('Variety', "Odrůda - Heirloom, Tabi...")]])],
     ...
     [sg.Text('0', key='_LEFT_', size=slidercount_size, font=base_font),                     # Robusta counter
      sg.Text('Robusta', size=slidertext_size, font=base_font,
              justification=base_align),   # Robusta name
      sg.Text('Arabica', size=slidertext_size, font=base_font,
              justification=base_align),   # Arabica name
...
autodelete = ['Country', 'Name', 'Roaster', 'Processing', 'Variety']
# List comprehension for space reduction
[windowBeans[key].bind('<FocusIn>', '+FOCUS IN+') for key in autodelete]
# windowBeans['Country'].bind('<FocusIn>', '+FOCUS IN+')    # can be deleted
# windowBeans['Name'].bind('<FocusIn>', '+FOCUS IN+')       # can be deleted
# windowBeans['Roaster'].bind('<FocusIn>', '+FOCUS IN+')    # can be deleted
# windowBeans['Processing'].bind('<FocusIn>', '+FOCUS IN+') # can be deleted
# windowBeans['Variety'].bind('<FocusIn>', '+FOCUS IN+')    # can be deleted
...
def input_tasting():
    ...
    for col in cols:
        if col == "Date":
            ...
            # Variety has numeric value, it's a float object and cannot be uppercased
            row_dict[col] = values[col]
        else:
            # Make all inputs start with uppercase
